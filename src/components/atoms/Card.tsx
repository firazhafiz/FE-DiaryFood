      ...
      <div className="bg-white rounded-2xl shadow-xs overflow-hidden hover:shadow-md transition-all duration-300 w-[250px] h-[320px] flex flex-col cursor-pointer hover:scale-[1] active:scale-95">
        ...
        <div className="flex-1 flex flex-col justify-between p-4 pb-2 h-full">
          <div>
            ...
            <h3 className="text-base font-semibold text-gray-900 mb-1 line-clamp-3">{title}</h3>
            <div className="flex items-center mb-5">
              <span className="font-bold text-[color:var(--custom-orange)] text-base">{formatTime(totalTime)}</span>
            </div>
          </div>
          <div className="flex items-center pt-2 border-t border-gray-200 gap-2">
            <Image src={author.avatar} alt={author.name} width={1000} height={1000} className="w-6 h-6 rounded-full object-cover" />
            <span className="text-gray-800 text-xs font-medium">{author.name}</span>
          </div>
        </div>
      </div>
    </Link>
