...
interface CategoryManagementProps {
  categories: Category[];
  onAddCategory: (newCategory: { nama: string }) => void;
...
const CategoryManagement = ({ categories, onAddCategory, onDeleteCategory }: CategoryManagementProps) => {
  ...
  return (
    <div className="p-6 relative min-h-[calc(100vh-200px)]">
        ...
        <div className="overflow-x-auto">
          <div className="min-w-full divide-y divide-gray-200">
            <div className="grid grid-cols-3 rounded-2xl py-4 mb-1">
              <div className="px-6 text-center text-md font-medium text-slate-700 tracking-wider">Nama</div>
              <div className="px-6 text-center text-md font-medium text-slate-700 tracking-wider">Jumlah Resep</div>
              <div className="px-6 text-center text-md font-medium text-slate-700 tracking-wider">Aksi</div>
            </div>

            <div className="space-y-4">
              {currentCategories.length === 0 ? (
                <div className="text-center py-4 text-slate-500">Tidak ada kategori</div>
              ) : (
                currentCategories.map((category) => (
                  <div key={category.id} className="grid grid-cols-3 bg-white/60 border-2 border-white rounded-2xl py-4">
                    <div className="px-6 whitespace-nowrap">
                      <div className="text-sm text-center font-medium text-gray-900">{category.nama}</div>
                    </div>
                    <div className="px-6 whitespace-nowrap">
                      <p className="px-2 py-1 text-xs text-center font-medium text-slate-900">{category.totalRecipes} resep</p>
                    </div>
                    <div className="px-6 whitespace-nowrap text-sm font-medium">
                      <div className="flex justify-center space-x-3">
                        ...
                        <button className="text-red-600 hover:text-red-800 transition-colors" onClick={() => onDeleteCategory(category.id)}>
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path
                              fillRule="evenodd"
      ...
      <AddCategoryModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onAdd={onAddCategory} />
    </div>
  );
};
