  ...
  return (
    <div className="p-6 relative min-h-[calc(100vh-200px)]">
      <div className="flex justify-center mb-6">
        <button onClick={() => setIsModalOpen(true)} className="bg-[#FF7A5C] text-white text-sm cursor-pointer px-6 py-2 rounded-md hover:bg-[#ff6b4a] transition-colors flex items-center gap-2 shadow-sm">
          ...
          <div className="min-w-full divide-y divide-gray-200">
            ...
            <div className="space-y-4">
              {currentCategories.length === 0 ? ( // BARU: Tangani kasus tidak ada data
                <div className="text-center py-4">Wait a minute...</div>
              ) : (
                currentCategories.map((category) => (
                  <div key={category.id} className="grid grid-cols-3 bg-white/60 border-2 border-white rounded-2xl py-4">
                    ...
                    <div className="px-6 whitespace-nowrap">
                      <p className="px-2 py-1 text-xs text-center font-medium text-slate-900">{category.totalRecipes} recipes</p>
                    </div>
