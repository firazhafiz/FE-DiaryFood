...
const DetailHeader = ({ recipe, loading }: DetailHeaderProps) => {
    ...
    const checkSavedStatus = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${config.apiUrl}/resep/${recipe.id}/saved-status`,
          {
            headers: {
  ...
  const handleUnsaved = async () => {
    ...
    try {
      ...
      if (!token) throw new Error("No authentication token found");
      const response = await fetch(
        `${config.apiUrl}/resep/${recipe.id}/unsave`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  // Gunakan placeholder jika photoResep null atau kosong
  const imageSrc = recipe.photoResep
    ? recipe.photoResep
    : "https://via.placeholder.com/400x200"; // URL placeholder eksternal
  ...
  return (
    <Suspense fallback={null}>
      <div className="mb-8">
        ...
        {!recipe.photoResep && (
          <div className="rounded-2xl overflow-hidden mb-4 w-full max-w-2xl bg-gray-200 h-[350px] flex items-center justify-center">
