  ...
  try {
    ...
    if (recipesResponse.ok) {
      ...
      if (Array.isArray(recipesData.data.reseps)) {
        relatedRecipes = recipesData.data.reseps.filter((r: Recipe) => r.id !== parsedRecipeId).slice(0, 2);
      }
    }
    ...
    return {
      recipe: null,
      recipes: [],
      error: err instanceof Error ? err.message : "Gagal mengambil detail resep",
    };
  }
}

export default async function DetailResep({ searchParams }: { searchParams: Promise<{ recipeId?: string }> }) {
  ...
  if (error || !recipe) {
    return (
      <div className="flex items-center justify-center min-h-screen flex-col gap-4">
        <div className={error ? "text-red-500 text-lg" : "text-gray-700 text-lg"}>{error || "Resep tidak ditemukan"}</div>
        <Link href="/" className="bg-[color:var(--custom-orange)] text-white px-4 py-2 rounded hover:bg-orange-600 transition-colors">
          Kembali ke Beranda
        </Link>
      </div>
    );
